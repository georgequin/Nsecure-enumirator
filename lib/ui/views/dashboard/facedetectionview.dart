...
class FaceRecognitionViewState extends State<FaceRecognitionView> {
  ...
  void initState() {
    super.initState();
    print('>>>>>>>>>>>>>>>>>>>>>>>>>just started fixing all issues...');
  ...
  Future<bool> onFaceDetected(faces) async {
    ...
    setState(() {
      print('>>>>>>>>>>>>>>>>>>>>>>>>>>>> updated face init');
      _faces = faces;
    });

    print('>>>>>>>>>>>>>>>>>>>>>>>>>>>> updated face:  $_faces');
    ...
    if (faces.length > 0) {
      ...
      if (maxLiveness > _livenessThreshold) {
        recognized = true;
        print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>updated recognition status');
      }
...
class _FaceDetectionViewState extends State<FaceDetectionView> {
  ...
  void _onPlatformViewCreated(int id) async {
    final prefs = await SharedPreferences.getInstance();
    var cameraLens = prefs.getInt("camera_lens");
    print('camera lense is $cameraLens');
    ...
    print('>>>>>>>>>>>>>>>>>>>>>>>> started creating detection controller');
  }
}
